...
public class SubscriptionDeletionWorkflowExecutor extends AbstractSubscriptionWorkflowExecutor {
    ...
    protected WorkflowResponse handleCommercialPlan(SubscriptionWorkflowDTO subscriptionWorkflowDTO)
        ...
        try {
            ...
            if (responseObj.isJsonObject()) {
                ...
                if (!isTestAccount) {
                    String accountNumber =
                            subscriberJsonObj.get(CustomWorkFlowConstants.ACCOUNT_NUMBER_PROPERTY).isJsonPrimitive() ?
                            subscriberJsonObj.get(CustomWorkFlowConstants.ACCOUNT_NUMBER_PROPERTY).getAsString() : null;
            ...
            throw new WorkflowException(ERROR_MSG + " Error while cancelling the subscription. Tenant: " +
                                        subscriptionWorkflowDTO.getTenantDomain() + ", Subscriber: " +
                                        subscriptionWorkflowDTO.getSubscriber() + ",  Application: " +
                                        subscriptionWorkflowDTO.getApplicationName() + ", Api: " +
                                        subscriptionWorkflowDTO.getApiName() + ", API version: " +
                                        subscriptionWorkflowDTO.getApiVersion(), e);
        }
    }
    ...
    private WorkflowResponse handleSubscriptionIfInactive(SubscriptionWorkflowDTO subscriptionWorkflowDTO)
        ...
        if (!isAnActiveSubscription(subscriptionWorkflowDTO)) {
            ...
            throw new WorkflowException(ERROR_MSG + " Subscriber information is not available. Tenant: " +
                                        ...
                                        subscriptionWorkflowDTO.getApplicationName() + ", Api: " +
                                        subscriptionWorkflowDTO.getApiName() + ", API version: " +
                                        subscriptionWorkflowDTO.getApiVersion());
        }
    }
    ...
    private boolean isAnActiveSubscription(SubscriptionWorkflowDTO subscriptionWorkflowDTO)
        ...
        APIIdentifier apiIdentifier =
                new APIIdentifier(subscriptionWorkflowDTO.getApiProvider(), subscriptionWorkflowDTO.getApiName(),
                                  subscriptionWorkflowDTO.getApiVersion());
    ...
    private WorkflowResponse cancelSubscription(String accountNumber, SubscriptionWorkflowDTO subscriptionWorkflowDTO)
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Cancelling subscription for Account: " + accountNumber + " Application name: " +
                         subscriptionWorkflowDTO.getApplicationName() + " Api name: " +
                         subscriptionWorkflowDTO.getApiName() + " Api version: " +
                         subscriptionWorkflowDTO.getApiVersion());
        }
        ...
        if (response.getChildrenWithLocalName(CustomWorkFlowConstants.SOAP_RETURN_ELEMENT).hasNext()) {
            ...
            if (responseElement.isJsonObject()) {
                ...
                if (resultObj.get(CustomWorkFlowConstants.RESPONSE_SUCCESS) != null &&
                    resultObj.get(CustomWorkFlowConstants.RESPONSE_SUCCESS).getAsBoolean()) {
                    JsonObject dataObj = resultObj.get(CustomWorkFlowConstants.RESPONSE_DATA).getAsJsonObject();
                    if (dataObj.get(CustomWorkFlowConstants.SUBSCRIPTION_INFO_NOT_AVAILABLE) != null &&
                        dataObj.get(CustomWorkFlowConstants.SUBSCRIPTION_INFO_NOT_AVAILABLE).getAsBoolean()) {
                        LOGGER.warn("Subscription information is not available. Account no: " + accountNumber +
                                    ", Application name: " + subscriptionWorkflowDTO.getApplicationName() +
                                    ", Api name: " + subscriptionWorkflowDTO.getApiName() + ", Version: " +
                                    subscriptionWorkflowDTO.getApiVersion());
                    }

                    if (dataObj.get(CustomWorkFlowConstants.MONETIZATION_TABLES_UPDATED) == null ||
                        !dataObj.get(CustomWorkFlowConstants.MONETIZATION_TABLES_UPDATED).getAsBoolean()) {
                    ...
                    throw new WorkflowException("Failed to cancel the Subscription. Response: " + response.toString());
                }
            } else {
    ...
    @Override public WorkflowResponse complete(WorkflowDTO workflowDTO) throws WorkflowException {
        ...
        try {
            ...
            int applicationIdID =
                    apiMgtDAO.getApplicationId(subWorkflowDTO.getApplicationName(), subWorkflowDTO.getSubscriber());
