...
public class ZuoraProductClient extends ZuoraClient {
    ...
    private static final String ERROR_JSON_OBJ_PRODUCT_RATEPLAN_UPDATE_FAILURE = "{\"code\": null,\"codeSpecified\": "
            + "true,\"field\": null,\"fieldSpecified\": false,\"message\": \"Product rate plan update failed. \",\""
    ...
    public JsonObject createProduct(JsonObject productInfo) throws CloudBillingZuoraException {
        ...
        String productCategory = productInfo.get(BillingConstants.JSON_OBJ_PRODUCT_CATEGORY).toString()
                .replaceAll("\"", "");
        try {
            ...
            throw new CloudBillingZuoraException("Invalid type fault error " + PRODUCT_CREATION_ERROR + productName,
                    errorCode, e);
    ...
    public JsonObject createProductRatePlan(JsonObject productRatePlanInfo) throws CloudBillingZuoraException {
        String productName = productRatePlanInfo.get(BillingConstants.JSON_OBJ_PRODUCT_NAME).toString()
                .replaceAll("\"", "");
        String productRatePlanName = productRatePlanInfo.get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_NAME).toString()
                .replaceAll("\"", "");
        String productRatePlanChargeTierPrice = productRatePlanInfo
                .get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_RECURRING_CHARGE).toString().replaceAll("\"", "");
        String productRatePlanOverageChargePrice = productRatePlanInfo
                .get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_OVERAGE_CHARGE).toString().replaceAll("\"", "");
        ...
        } catch (CloudBillingZuoraException | InvalidQueryLocatorFault | MalformedQueryFault | ParseException e) {
    ...
    public JsonObject updateProductRatePlan(JsonObject productInfo) throws CloudBillingZuoraException {
        String productName = productInfo.get(BillingConstants.JSON_OBJ_PRODUCT_NAME).toString().replaceAll("\"", "");
        String productRatePlanName = productInfo.get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_NAME).toString()
                .replaceAll("\"", "");
        String productRatePlanRecurringCharge = productInfo
                .get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_RECURRING_CHARGE).toString().replaceAll("\"", "");
        String productRatePlanOverageUsageCharge = productInfo
                .get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_OVERAGE_CHARGE).toString().replaceAll("\"", "");
        String description = productInfo.get(BillingConstants.JSON_OBJ_PRODUCT_RATEPLAN_DESCRIPTION).toString()
                .replaceAll("\"", "");
        // Create error object
        ...
        try {
            ...
            if (product != null) {
                ...
                if (!productRatePlanRecurringCharge.isEmpty()) {
                    ...
                    ProductRatePlanCharge productRatePlanCharge = getProductRatePlanCharge(
                            BillingConstants.RATEPLAN_CHARGE_NAME_MONTHLY_SUBSCRIPTION, productRatePlanId.toString());
                    productRatePlanCharge.setProductRatePlanId(productRatePlanId);
                    // Get the product rate plan charge id for recurring charges
                    ID productRatePlanChargeId = productRatePlanCharge.getId();
                    ProductRatePlanChargeTier chargeTier = getProductRatePlanChargeTier(
                            productRatePlanChargeId.toString());
                ...
                if (!productRatePlanOverageUsageCharge.isEmpty()) {
                    ...
                    ProductRatePlanCharge productOverUsageCharge = getProductRatePlanCharge(
                            BillingConstants.RATEPLAN_CHARGE_NAME_OVERUSAGE, productRatePlanId.toString());
                    productOverUsageCharge.setProductRatePlanId(productRatePlanId);
                    // Get the product rate plan charge id for over usage charges
                    ID productOverUsageChargeId = productOverUsageCharge.getId();
                    ProductRatePlanChargeTier overUsageChargeTier = getProductRatePlanChargeTier(
                            productOverUsageChargeId.toString());
                    overUsageChargeTier
            ...
            errorObjs = new JsonObject[] {
                    new JsonParser().parse(ERROR_JSON_OBJ_PRODUCT_RATEPLAN_UPDATE_FAILURE).getAsJsonObject() };
            errorResponse.add(BillingConstants.EEROR_RESPONSE_PROPERTY_ERRORS, new Gson().toJsonTree(errorObjs));
            return errorResponse;
        }
    }
