...
/**
 * Reading and Writing class for SSL files in Nginx for each custom URL
 */
    ...
    private void writeFile(String filePath, String fileContent) throws IOException {
        ...
        try {
            bufferedWriter = new BufferedWriter(
                    new OutputStreamWriter(new FileOutputStream(filePath), NginxVhostConstants.DEFAULT_ENCODING));
    ...
    public File storeFileInLocal(String fileType, String tenantDomain, String type)
        ...
        try {
            ...
            if (NginxVhostConstants.CERTIFICATE_FILE.equals(fileType)) {
                ...
                String sslPublicKeyContent = getDecryptContent(
                        new String(this.retrieveFileFromRegistry(publicKeyFileLocation),
                                   NginxVhostConstants.DEFAULT_ENCODING));
            ...
            if (file.getParentFile().mkdirs()) {
                if (log.isDebugEnabled()) {
                    log.debug("Directory structure created to store files");
                }
            }
        ...
        try {
            while (!this.isFileAvailableInRegistry(registryPath) && retryCount < 3) {
                try {
                    ...
                    throw new DomainMapperException(ex);
                }
            }
