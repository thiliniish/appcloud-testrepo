...
import org.wso2.cloud.heartbeat.monitor.modules.apimanager.data.GenerateAppKeyRequest;
import org.wso2.cloud.heartbeat.monitor.modules.apimanager.data.GetSubscriptionsRequest;
import org.wso2.cloud.heartbeat.monitor.modules.apimanager.data.SubscriptionRequest;
import org.wso2.cloud.heartbeat.monitor.modules.apimanager.data.UnsubscriptionRequest;
import org.wso2.cloud.heartbeat.monitor.modules.apimanager.util.VerificationUtil;
...
public class APIStoreRestClient {
    ...
    public HttpResponse getSubscriptions(GetSubscriptionsRequest getSubscriptionsRequest)
            throws Exception {
        checkAuthentication();
        HttpResponse response = HttpRequestUtil.doPost(new URL(backEndUrl + "/store/site/blocks/subscription/subscription-list/ajax/subscription-list.jag")
                , getSubscriptionsRequest.generateRequestParameters()
                , requestHeaders);
        if (response.getResponseCode() == 200) {
            VerificationUtil.checkErrors(response);
            return response;
    ...
    public HttpResponse unsubscribe(UnsubscriptionRequest unsubscriptionRequest)
            throws Exception {
        checkAuthentication();
