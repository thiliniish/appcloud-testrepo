...
public abstract class AbstractGitTest implements Job {
    ...
    public void init() throws IOException {
        ...
        testInfo = new TestInfo(serviceName, testName, remotePath, severity);
    }
    ...
    protected void initRepo() throws IOException {
        ...
        try {
            ...
            String msg = "IOException occurred while creating local repository " + localPath;
            log.error(msg);
    ...
    protected void gitClone() throws Exception {
        ...
        String repo =
                remotePath + File.separator + Constants.GIT + File.separator + applicationKey +
                Constants.REPO_TYPE;
        try {
            UsernamePasswordCredentialsProvider credentialsProvider =
                    new UsernamePasswordCredentialsProvider(gitUser, gitPassword);
            Git.cloneRepository().setCredentialsProvider(credentialsProvider).setURI(repo)
               .setDirectory(new File(localPath)).call();
            log.info("Executing Git Clone done " + remotePath + " to " + localPath);

        } catch (Exception e) {
            throw new Exception(
                    "GitAPIException while executing git clone from repository : " + repo +
                    " to : " + localPath, e) { // for
                // repo
            };
        }
     ...
     * @throws org.eclipse.jgit.errors.GitAPIException
     */
    protected void gitAdd() throws Exception {
        initRepo();
        if (log.isDebugEnabled()) {
        ...
        try {
            ...
            throw new Exception("GitAPIException while executing git add", e) {
            };
        }
    }
     ...
     * @throws org.eclipse.jgit.errors.GitAPIException
     */
    protected void gitCommit() throws Exception {
        initRepo();
        if (log.isDebugEnabled()) {
        ...
        try {
            ...
            throw new Exception("GitAPIException while executing git commit", e) {
            };
        }
    }
     ...
     * @throws org.eclipse.jgit.errors.GitAPIException
     */
    protected void gitPush() throws Exception {
        initRepo();
        if (log.isDebugEnabled()) {
        ...
        try {
            ...
            throw new Exception("GitAPIException while executing git push", e) {
            };
        }
    }
    ...
    protected boolean repoExists(String applicationKey) throws IOException {
        ...
        try {
            ...
            String msg = "Repository is not deleted : " + repoName + " due to " +
    ...
    private RepositoryModel findRepository(String name, String url, String account, String password)
        ...
        Map<String, RepositoryModel> repositories =
                RpcUtils.getRepositories(url, account, password.toCharArray());
    ...
    protected void cleanDirectory(String path) {
        ...
        if (directory.exists()) {
            try {
                ...
                log.error("Exception occurred while deleting git temp directory ", e);
            }
            directory.delete();
        }
    ...
    protected void handleError(String type, Object obj) {
        ...
        if (type.equals("GitAPIException")) {
            ...
            log.error(CaseConverter.splitCamelCase(serviceName) + " - " + hostName,
                      gitAPIException);
            testStateHandler.onFailure(testInfo, gitAPIException.getMessage(), gitAPIException);
        } else if (type.equals("IOException")) {
            IOException ioException = (IOException) obj;
            log.error(CaseConverter.splitCamelCase(serviceName) + hostName, ioException);
            testStateHandler.onFailure(testInfo, ioException.getMessage(), ioException);
        } else if (type.equals("TransportException")) {
            TransportException tException = (TransportException) obj;
            log.error(CaseConverter.splitCamelCase(serviceName) + hostName, tException);
            testStateHandler.onFailure(testInfo, tException.getMessage(), tException);
        } else if (type.equals("Exception")) {
            Exception exception = (Exception) obj;
            log.error(CaseConverter.splitCamelCase(serviceName) + hostName, exception);
            testStateHandler.onFailure(testInfo, exception.getMessage(), exception);
        }
    }
