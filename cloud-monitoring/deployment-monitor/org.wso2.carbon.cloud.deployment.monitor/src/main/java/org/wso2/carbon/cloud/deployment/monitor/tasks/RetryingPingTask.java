...
import org.wso2.deployment.monitor.impl.task.PingTask;

import java.util.Properties;

/**
 * Retrying Implementation for {@link PingTask}
 */
public class RetryingPingTask extends AbstractRetryingTask implements DeploymentMonitorTask {

    @Override public RunStatus runTask(ServerGroup serverGroup, Properties properties) {
        PingTask pingTask = new PingTask();
        RunStatus status = pingTask.execute(serverGroup, properties);
        if (status.isSuccess()) {
            return status;
        } else {
            return retryTask(serverGroup, properties, "Ping Task Failed for : " + status.getServerGroupName(), status);
        }
    }
